...
| `shutdown_drain_timeout_secs` | integer | `30`            | Graceful-drain deadline in seconds; in-flight streams still running after it are force-cancelled. Time wait defined is needed for a graceful shutdown. Matches Kubernetes `terminationGracePeriodSeconds` default |
